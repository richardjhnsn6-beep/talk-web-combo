...
export default function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e) => {
      ...
      // Show prompt immediately if not in standalone mode
    ...
    // Show iOS prompt immediately if applicable
  ...
  const handleDismiss = () => {
    setShowPrompt(false);
    // Don't store dismissal - allow it to reappear on next visit
  };
  ...
  return (
    <>
      ...
      {deferredPrompt && !isIOS && (
        <div className="fixed top-20 sm:bottom-24 sm:top-auto right-4 sm:right-6 w-80 max-w-[calc(100vw-2rem)] bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl shadow-2xl p-3 z-45 animate-slide-up">
          <button
